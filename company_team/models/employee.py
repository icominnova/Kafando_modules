...
class CompanyEmployee(models.Model):
    ...
    @api.constrains("department_id", "team_id")
    def _check_department_team(self):
        for record in self:
            if record.department_id and record.team_id:
                if record.team_id.department_id != record.department_id:
                    raise ValidationError(
                        "L'équipe doit appartenir au même département que l'employé."
                    )   